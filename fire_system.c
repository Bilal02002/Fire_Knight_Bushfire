...
void all_off(void) {
    ...
    if (ret1 == ESP_OK && ret2 == ESP_OK) {
        printf("[FIRE_SYSTEM] All pumps turned OFF\n");
    } else {
...
void init_current_sensors(void) {
    ...
    if (ret != ESP_OK) {
        ...
        printf("[FIRE_SYSTEM] ADC calibration initialized successfully\n");
    }
    ...
    printf("[FIRE_SYSTEM] Current sensors initialized successfully\n");
}
...
void check_automatic_activation(void) {
    ...
    if (shouldActivateFullSystem) {
        ...
        // ========================================
        printf("[FIRE] FULL-SYSTEM MODE: All 4 sensors confirm fire\n");
...
void init_door_sensor(void) {
    ...
    if (doorOpen) {
        printf("[FIRE_SYSTEM] Door: OPEN (initial state)\n");
        doorOpenTime = xTaskGetTickCount() * portTICK_PERIOD_MS;
    } else {
        printf("[FIRE_SYSTEM] Door: CLOSED (initial state)\n");
    }
}
...
void init_fire_suppression_system(void) {
    ...
    snprintf(log_msg, LOG_BUFFER_SIZE, 
            "[FIRE_SYSTEM] Initializing PCA9555 at address 0x%02X...", 
            PCA9555_I2C_ADDRESS);
    printf("%s\n", log_msg);
    
    snprintf(log_msg, LOG_BUFFER_SIZE, 
            "[FIRE_SYSTEM] I2C Pins: SDA=GPIO%d, SCL=GPIO%d", 
            PCA9555_I2C_SDA_GPIO, PCA9555_I2C_SCL_GPIO);
    printf("%s\n", log_msg);
    ...
    if (ret != ESP_OK) {
        ...
        all_off();
        printf("[FIRE_SYSTEM] All pumps initialized to OFF state\n");
    }
    ...
    printf("[FIRE_SYSTEM] Camera control pin initialized\n");
