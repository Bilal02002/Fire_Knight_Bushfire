...
static esp_err_t gsm_get_imsi(char *imsi_out, size_t max_len)
{
    ...
    for (int attempt = 0; attempt < retries; attempt++) {
        ...
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "IMSI read failed (attempt %d/%d)", attempt + 1, retries);
        ...
        if (strlen(imsi_out) >= 6) {
            ESP_LOGI(TAG, "IMSI: %s", imsi_out);
            ESP_LOGI(TAG, "MCC+MNC: %.5s", imsi_out);
            return ESP_OK;
        }

        ESP_LOGW(TAG, "Invalid IMSI length: %d (attempt %d/%d)", strlen(imsi_out), attempt + 1, retries);
        vTaskDelay(pdMS_TO_TICKS(1000));
    }

    ESP_LOGE(TAG, "Failed to get valid IMSI after %d attempts", retries);
    return ESP_FAIL;
}
        ...
        if (apn_start) {
            ...
            if (apn_end && (apn_end - apn_start) > 0 && (apn_end - apn_start) < APN_MAX_LENGTH) {
                ...
                if (strlen(detected_apn) > 0) {
                    ESP_LOGI(TAG, "Found APN from SIM: %s", detected_apn);
                    return ESP_OK;
                }
            }
        }
...
{
    ESP_LOGI(TAG, "AUTOMATIC APN DETECTION");

    // ========== METHOD 1: Module's Automatic Detection ==========
    ESP_LOGI(TAG, "Method 1: Trying module auto-detection...");
    ...
    if (err == ESP_OK) {
        ...
        if (gsm_read_apn_from_sim() == ESP_OK && strlen(detected_apn) > 0) {
            ESP_LOGI(TAG, "Module auto-detected: %s", detected_apn);
            apn_detected = true;
            return ESP_OK;
        }
    }

    ESP_LOGW(TAG, "Module auto-detection didn't provide APN");

    // ========== METHOD 2: Read from SIM Card ==========
    ESP_LOGI(TAG, "Method 2: Reading from SIM card...");

    if (gsm_read_apn_from_sim() == ESP_OK && strlen(detected_apn) > 0) {
        ESP_LOGI(TAG, "Got APN from SIM: %s", detected_apn);
        apn_detected = true;
        return ESP_OK;
    }

    // ========== METHOD 3: Lookup by IMSI ==========
    ESP_LOGI(TAG, "Method 3: Looking up by IMSI...");
    ESP_LOGI(TAG, "Waiting for SIM to stabilize...");
    ...
    if (gsm_get_imsi(imsi, sizeof(imsi)) == ESP_OK) {
        ...
        if (apn_config != NULL) {
            ...
            ESP_LOGI(TAG, "Found in database:");
            ESP_LOGI(TAG, "Operator: %s", apn_config->operator_name);
            ESP_LOGI(TAG, "APN: %s", detected_apn);
            if (strlen(detected_username) > 0) {
                ESP_LOGI(TAG, "Username: %s", detected_username);
            }

            apn_detected = true;
            return ESP_OK;
        } else {
            ESP_LOGW(TAG, "Operator not in database (MCC+MNC: %.6s)", imsi);
        }
    }

    //Method 4
    ESP_LOGI(TAG, "Method 4: Querying operator code...");

    memset(response, 0, sizeof(response));
    err = esp_modem_at_raw(dce, "AT+COPS?\r", response, "OK", "ERROR", 5000);

    if (err == ESP_OK) {
        ESP_LOGD(TAG, "AT+COPS response: %s", response);
        ...
        if (cops_start) {
            ...
            if (quote_start) {
                ...
                if (quote_end && (quote_end - quote_start) >= 5 && (quote_end - quote_start) <= 6) {
                    ...
                    ESP_LOGI(TAG, "Operator code extracted: %s", operator_code);
                    ...
                    if (apn_config != NULL) {
                        ...
                        ESP_LOGI(TAG, "Found from operator code:");
                        ESP_LOGI(TAG, "Operator: %s", apn_config->operator_name);
                        ESP_LOGI(TAG, "APN: %s", detected_apn);
                        ...
                        ESP_LOGW(TAG, "Operator %s not in database", operator_code);
                    }
                } else {
                    ESP_LOGW(TAG, "Invalid operator code length");
                }
            } else {
                ESP_LOGW(TAG, "No quoted string found in COPS response");
            }
        } else {
            ESP_LOGW(TAG, "+COPS: not found in response");
        }
    } else {
        ESP_LOGE(TAG, "AT+COPS? command failed");
    }

    // ========== METHOD 5: Try common generic APNs ==========
    ESP_LOGI(TAG, "Method 5: Trying common generic APNs...");
    ...
    for (int i = 0; generic_apns[i] != NULL; i++) {
        ESP_LOGI(TAG, "Testing: %s", generic_apns[i]);
        strncpy(detected_apn, generic_apns[i], APN_MAX_LENGTH - 1);

        // We'll use this as a last resort - don't validate connectivity here
        apn_detected = true;
        ESP_LOGI(TAG, "Using fallback APN: %s", detected_apn);
        return ESP_OK;
    }

    ESP_LOGE(TAG, "All detection methods failed");
    return ESP_FAIL;
}
                        ...
                        int32_t event_id, void *event_data)
{
    ...
    if (event_id == IP_EVENT_PPP_GOT_IP) {
        ...
        ESP_LOGI(TAG, "DNS Primary : 8.8.8.8");
        ...
        ESP_LOGI(TAG, "DNS Secondary: 1.1.1.1");

        // Wait for PPP link to fully stabilize before configuring DNS
        ESP_LOGI(TAG, "Waiting for PPP link stabilization...");
        ...
        if (dns_test == 0 && result != NULL) {
            ESP_LOGI(TAG, "DNS Working - google.com resolved");
            freeaddrinfo(result);
        } else {
            ESP_LOGW(TAG, "DNS Test Failed (code: %d)", dns_test);
        }
        ...
        ESP_LOGI(TAG, "Notifying time manager of GSM disconnection...");
...
static void on_ppp_changed(void *arg, esp_event_base_t event_base,
                           int32_t event_id, void *event_data)
{
    ESP_LOGI(TAG, "PPP state changed: %" PRIu32, event_id);

    switch (event_id) {
        case NETIF_PPP_ERRORNONE:
            ESP_LOGI(TAG, "PPP: No error");
            break;
        case NETIF_PPP_ERRORAUTHFAIL:
            ESP_LOGI(TAG, "PPP: Authentication failed");
            break;
        case NETIF_PPP_ERRORPEERDEAD:
            ESP_LOGI(TAG, "PPP: Peer dead");
            break;
        case NETIF_PPP_ERRORIDLETIMEOUT:
            ESP_LOGI(TAG, "PPP: Idle timeout");
            break;
        case NETIF_PPP_ERRORCONNECT:
            ESP_LOGI(TAG, "PPP: Connection error");
            break;
        case NETIF_PPP_ERRORUSER:
            ESP_LOGI(TAG, "PPP: User stopped");
            break;
        default:
            ESP_LOGI(TAG, "PPP event: %ld", (unsigned long) event_id);
            break;
    }
}
...
static esp_err_t gsm_modem_hardware_reset(void)
{
    ...
    ESP_LOGI(TAG, "Step 4: Waiting for module boot...");
...
static esp_err_t gsm_force_command_mode(void)
{
    ...
    ESP_LOGI(TAG, "Attempting to exit DATA mode...");
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Escaped from DATA mode with +++");
    ...
    ESP_LOGE(TAG, "Could not exit DATA mode - hardware reset needed");
    return ESP_FAIL;
}
...
esp_err_t gsm_manager_init(void)
{
    ...
    ESP_LOGI(TAG, "Performing hardware reset before initialization...");
    esp_err_t reset_result = gsm_modem_hardware_reset();
    if (reset_result != ESP_OK) {
        ESP_LOGW(TAG, "Hardware reset failed, continuing anyway...");
    }
    ...
    gsm_active = true;
    ESP_LOGI(TAG, "GSM modem initialized successfully");
...
esp_err_t gsm_manager_connect(void)
{
    if (!gsm_active || dce == NULL) {
        ESP_LOGE(TAG, "GSM not initialized");
        return ESP_FAIL;
    }
    ...
    ESP_LOGI(TAG, "Ensuring module is in COMMAND mode...");
    gsm_force_command_mode();
    vTaskDelay(pdMS_TO_TICKS(500));

    // ========== STEP 0: Wake up modem with multiple AT attempts ==========
    ESP_LOGI(TAG, "Waking up modem...");
    char response[128] = {0};
    esp_err_t err = ESP_FAIL;
    ...
    for (int wake_attempts = 0; wake_attempts < 5; wake_attempts++) {
        ...
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "Modem awake (attempt %d)", wake_attempts + 1);
            break;
        }

        ESP_LOGW(TAG, "Modem not responding, retrying... (%d/5)", wake_attempts + 1);
        vTaskDelay(pdMS_TO_TICKS(1000));
    }

    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Modem not responding after 5 attempts");
        return ESP_FAIL;
    }
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Modem status: %s", response);
    }
    ...
    if (err == ESP_OK) {
        ...
        if (rssi == 99) {
            ESP_LOGE(TAG, "No signal detected");
            return ESP_FAIL;
        } else if (rssi < 8) {
            ESP_LOGW(TAG, "Very weak signal (rssi=%d) - may fail", rssi);
            // Don't fail, but warn
        }
    }
    ...
    if (err == ESP_OK) {
        if (strstr(response, "READY") != NULL) {
            ...
            ESP_LOGI(TAG, "SIM requires PIN");
            // Add PIN handling here if needed
        } else {
            ESP_LOGW(TAG, "SIM status: %s", response);
    ...
    while (reg_attempts < 30) {  // 30 attempts * 2 seconds = 60 seconds max
        ...
        if (err == ESP_OK) {
            ...
            if (creg_ptr) {
                ...
                ESP_LOGI(TAG, "Status: %s (code=%d, attempt=%d)", status_str, reg_status, reg_attempts + 1);

                if (reg_status == 1 || reg_status == 5) {
                    ESP_LOGI(TAG, "Registered to network!");
                    break;
                } else if (reg_status == 3) {
                    ESP_LOGE(TAG, "Registration denied by network");
                    return ESP_FAIL;
                }
            }
        } else {
            ESP_LOGE(TAG, "CREG command failed");
        }
    ...
    ESP_LOGI(TAG, "Waiting for SIM filesystem initialization...");
    vTaskDelay(pdMS_TO_TICKS(3000));  // ← KEY FIX

    if (!apn_detected) {
        if (gsm_detect_apn() != ESP_OK) {
            ESP_LOGE(TAG, "APN detection failed completely!");
            return ESP_FAIL;
        }
    } else {
        ESP_LOGI(TAG, "Using previously detected APN: %s", detected_apn);
    }

    // ========== STEP 6: Wait for GPRS registration ==========
    ESP_LOGI(TAG, "Checking GPRS registration...");
    int gprs_attempts = 0;
    int gprs_status = -1;
    ...
    while (gprs_attempts < 20) {
        ...
        if (err == ESP_OK) {
            ...
            if (cgreg_ptr) {
                ...
                ESP_LOGI(TAG, "GPRS status: %d (attempt %d)", gprs_status, gprs_attempts + 1);
    ...
    if (gprs_status != 1 && gprs_status != 5) {
        ESP_LOGW(TAG, "GPRS not registered, trying anyway...");
    }
    ...
    if (err == ESP_OK) {
        ...
        if (cgatt_ptr) {
            ...
            ESP_LOGI(TAG, "GPRS attached: %s", attached ? "YES" : "NO");

            if (!attached) {
                ESP_LOGI(TAG, "Attaching to GPRS...");
    ...
    ESP_LOGI(TAG, "Configuring PDP context with detected APN...");
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "PDP context configured with APN: %s", detected_apn);
    } else {
        ESP_LOGW(TAG, "PDP context configuration warning");
    }

    // Verify PDP context
    memset(response, 0, sizeof(response));
    esp_modem_at_raw(dce, "AT+CGDCONT?\r", response, "OK", "ERROR", 5000);
    ESP_LOGI(TAG, "Current PDP: %s", response);

    // ========== STEP 8.5: ACTIVATE PDP CONTEXT (CRITICAL!) ==========
    ESP_LOGI(TAG, "Deactivating any existing PDP context...");
    ...
    ESP_LOGI(TAG, "PDP status: %s", response);

    // ========== STEP 9: Switch to data mode with retry ==========
    ESP_LOGI(TAG, "Switch to data mode...");
    for (int retry = 0; retry < 3; retry++) {
        err = esp_modem_set_mode(dce, ESP_MODEM_MODE_DATA);
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "Successfully switched to data mode");
            break;
        } else {
            ESP_LOGE(TAG, "Failed to switch to data mode (attempt %d/3): %s",
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "All attempts to switch to data mode failed");
        return ESP_FAIL;
    }

    // ========== STEP 10: Wait for PPP connection ==========
    ESP_LOGI(TAG, "Waiting for PPP connection (timeout: 90s)...");
    ...
    if (bits & GSM_CONNECTED_BIT) {
        ESP_LOGI(TAG, "GSM connected successfully!");
        vTaskDelay(pdMS_TO_TICKS(2000));
        return ESP_OK;
    } else {
        ESP_LOGE(TAG, "GSM connection timeout or failed");
        ...
        ESP_LOGI(TAG, "Error report: %s", response);
...
int gsm_manager_get_signal_quality(void)
{
    ...
    if (gsm_connected) {
        ESP_LOGI(TAG, "Using cached signal: %d", cached_signal_rssi);
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Signal query failed");
