...
typedef struct {
    ...
    union {
		...
		struct {
		    ...
		    int remainingTime;          // ADD THIS LINE
		} pumpExtend;
...
static void send_alert_pump_extend_time(int pumpIndex, int extensionDuration, int remainingTime);
	    ...
	    if (extendValue != last_shadow_extend_time[i]) {
	        ...
	        if (extendValue >= 0 && extendValue <= 3 && pumps[i].timerProtected) {
	            ...
	            if (xSemaphoreTake(mutexPumpState, pdMS_TO_TICKS(100)) == pdTRUE) {
	                ...
	                if (extensionMs > 0) {
					    // Get the current remaining time before extending
					    unsigned long remaining = get_timer_remaining(i);
					    
					    extend_timer_protection(i, extensionMs);
					    
					    last_shadow_extend_time[i] = extendValue;
					    last_reported_extend_time[i] = extendValue;
					    state_changed = true;
					    ...
					    send_alert_pump_extend_time(i, extensionMs / 1000, (int)remaining);              
...
static void send_system_status(void) {
    ...
    bool current_faults[4] = {false};
    bool suppression_active = false;
    PumpState pump_states[4] = {PUMP_OFF};
    ...
    if (xSemaphoreTake(mutexSensorData, pdMS_TO_TICKS(100)) == pdTRUE) {
        ...
        // Extract current sensor values and fault status
        for (int i = 0; i < 4; i++) {
    ...
    cJSON_AddNumberToObject(payload, "waterLevel", round(level_s * 100.0) / 100.0);
    cJSON_AddNumberToObject(payload, "batteryVoltage", round(bat_v * 100.0) / 100.0);
    cJSON_AddNumberToObject(payload, "solarVoltage", round(sol_v * 100.0) / 100.0);
    ...
    // ========================================
    // ✅ NEW: GROUPED PUMP DATA STRUCTURE
    // ========================================
...
static void check_state_changes(void) {
    ...
    static bool pumps_actually_running = false;
    
    if (startAllPumpsActive != last_start_all_pumps) {
        if (startAllPumpsActive) {
            send_alert_start_all_pumps_activated();
            pumps_actually_running = true;
        }
        // Don't send deactivation alert here - wait for pumps to actually stop
        last_start_all_pumps = startAllPumpsActive;
    }
    
    if (!startAllPumpsActive && pumps_actually_running) {
        // Check if all pumps have stopped
        bool any_pump_running = false;
        for (int i = 0; i < 4; i++) {
            if (pumps[i].state == PUMP_MANUAL_ACTIVE && 
                pumps[i].activationSource == ACTIVATION_SOURCE_SHADOW_SINGLE) {
        ...
        if (!any_pump_running) {
            // All pumps have actually stopped
           
            send_alert_start_all_pumps_deactivated("TIMER_EXPIRED", 90);
            pumps_actually_running = false;
        }
    }
    ...
    // Check pump state changes (Now includes auto/manual mode flags)
    for (int i = 0; i < 4; i++) {
        ...
        if (current_state != last_pump_states[i]) {
            ...
            if (current_state == PUMP_AUTO_ACTIVE) {
                ...
                // Get previous runtime for logging
                if (last_pump_states[i] == PUMP_AUTO_ACTIVE || 
                    last_pump_states[i] == PUMP_MANUAL_ACTIVE) {
            ...
            // Send alert with all data (including auto/manual mode flags)
        ...
        if (current_fire_count == 1) {
            ...
            for (int i = 0; i < 4; i++) {
                if (fire_detected[i] && !fire_alerts_active[i]) {
                    ...
                    break;  // Only send one alert for single sector
                }
            }
        } else {
...
static void process_alerts(void) {
    ...
    while (xQueueReceive(alert_queue, &alert, 0) == pdTRUE) {
        ...
        switch(alert.type) {
		    ...
		    // ✅ NEW: Add auto/manual mode flags
		    cJSON_AddBoolToObject(payload, "autoEnabled", alert.data.pump.autoEnabled);
		    cJSON_AddBoolToObject(payload, "manualEnabled", alert.data.pump.manualEnabled);
		    ...
		    if (alert.data.pump.currentState == 1) { // AUTO_ACTIVE
		        ...
		        if (strlen(alert.data.pump.activationSource) > 0) {
		            cJSON_AddStringToObject(payload, "activationSource", 
		                alert.data.pump.activationSource);
		        }
		    } else if (alert.data.pump.currentState == 0) { // OFF
		        if (strlen(alert.data.pump.stopReason) > 0) {
		            cJSON_AddStringToObject(payload, "stopReason", 
		                alert.data.pump.stopReason);
		        }
		        ...
		        if (alert.data.pump.previousRuntime > 0) {
		            cJSON_AddNumberToObject(payload, "previousRuntime", 
		                alert.data.pump.previousRuntime);
		        }
		    }
		    break;
            ...
            case ALERT_TYPE_PUMP_EXTEND_TIME:
			    ...
			    cJSON_AddNumberToObject(payload, "remainingTime", alert.data.pumpExtend.remainingTime + alert.data.pumpExtend.extensionDuration );
                break;
		    ...
		    {
		        // ✅ Determine fire type flags
		        bool isSingleSector = (alert.data.fire.fireType == FIRE_TYPE_SINGLE_SECTOR);
		        bool isMultipleSectors = (alert.data.fire.fireType == FIRE_TYPE_MULTIPLE_SECTORS);
		        bool isFullSector = (alert.data.fire.fireType == FIRE_TYPE_FULL_SYSTEM);
		        
		        // Add fire type boolean flags
		        cJSON_AddBoolToObject(payload, "Single Sector", isSingleSector);
		        cJSON_AddBoolToObject(payload, "Multiple Sectors", isMultipleSectors);
		        cJSON_AddBoolToObject(payload, "Full Sector", isFullSector);
		        ...
		        // Add water level and estimated runtime
			...
			case ALERT_TYPE_MULTIPLE_FIRES:
			    {
			        ...
			        bool isMultipleSectors = (alert.data.multipleFires.fireType == FIRE_TYPE_MULTIPLE_SECTORS);
			        bool isFullSector = (alert.data.multipleFires.fireType == FIRE_TYPE_FULL_SYSTEM);
...
// ==========================================
static void send_alert_pump_extend_time(int pumpIndex,
                                       int extensionDuration,
                                       int remainingTime) {
    Alert alert = {0};
    ...
    snprintf(alert.message, sizeof(alert.message),
            "Extended %s by %d seconds (was %d seconds remaining)", 
            pumps[pumpIndex].name, extensionDuration, remainingTime);
    ...
    alert.data.pumpExtend.remainingTime = remainingTime;  // ADD THIS LINE
...
static void send_alert_fire_detected(int sensorIndex, const char* sectorName, 
    ...
    snprintf(alert.message, sizeof(alert.message),
            "FIRE DETECTED! 1 active fire sector | Type: %s", 
            get_fire_detection_type_string(fireInfo->type));
    
    // ✅ Store activeFireCount as 1
    alert.data.fire.sensorId = 1;  // Reusing this field for activeFireCount
    ...
    // ✅ Store fire type info (for boolean flags)
...
static void send_alert_multiple_fires(int fireCount, float sensorValues[4], 
    ...
    snprintf(alert.message, sizeof(alert.message),
            "FIRE DETECTED! %d active fire sectors | Type: %s", 
            fireCount, fireTypeStr);
    
    alert.data.multipleFires.activeFireCount = fireCount;
    
    // ✅ Add fire type info (for boolean flags)
...
#include "gsm_manager.h"
#include "config.h"
#include "ota_job.h"         // OTA update support
...
static void send_registration(void);
static void send_heartbeat(void);
void send_ota_alert(const char *otastatus, const char *version);
static void send_system_status(void);
bool enqueue_mqtt_publish(const char *topic, const char *payload);
static void check_provisioning_status(void);
static esp_err_t start_provisioning(void);
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_DATA:
            if (event->topic && event->data) {
                ...
                // Check if this is an OTA jobs message
                if (strstr(topic, "/jobs/") != NULL) {
                    printf("\n[OTA] Received OTA job message");
                    
                    // Create buffers for OTA processing
                    char topic_buf[128];
                    snprintf(topic_buf, sizeof(topic_buf), "%.*s", 
                            (int)event->topic_len, event->topic);
                    
                    // Allocate buffer for payload (max 4KB for OTA messages)
                    char *payload_buf = (char*)malloc(4096);
                    if (payload_buf) {
                        int copy_len = (event->data_len < 4095) ? event->data_len : 4095;
                        memcpy(payload_buf, event->data, copy_len);
                        payload_buf[copy_len] = '\0';
                        
                        // Process OTA message
                        ota_job_process_message(topic_buf, payload_buf, copy_len);
                        
                        free(payload_buf);
                    } else {
                        printf("\n[OTA] ERROR: Failed to allocate payload buffer");
                    }
                }
                // ==========================================
...
// ==========================================
// MQTT EVENT HANDLER - FIXED VERSION (with OTA logging)
// ==========================================
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_DATA:
            if (event->topic && event->data) {
                ...
                // ========================================
                // OTA TOPIC HANDLING WITH LOGGING
                // =======================================

				// ========================================
                // SPECIFIC LOGGING FOR jobs/get/accepted TOPIC
                // ========================================
                if (strstr(topic, "/jobs/get/accepted") != NULL) {
                    
                    printf("\n[OTA-JOBS] ========================================");
                    printf("\n[OTA-JOBS] JOBS/GET/ACCEPTED MESSAGE RECEIVED");
                    printf("\n[OTA-JOBS] ========================================");
                    printf("\n[OTA-JOBS] Full Topic: %.*s", event->topic_len, event->topic);
                    printf("\n[OTA-JOBS] Topic Length: %d bytes", event->topic_len);
                    printf("\n[OTA-JOBS] Payload Length: %d bytes", event->data_len);
                    
                    // Print the complete payload
                    if (event->data_len > 0) {
                        printf("\n[OTA-JOBS] Complete Payload:");
                        printf("\n%.*s", event->data_len, event->data);
                    }
                    
                    // Print formatted JSON if available
                    char *json_str = cJSON_Print(json);
                    if (json_str) {
                        printf("\n[OTA-JOBS] Formatted JSON Structure:");
                        printf("\n%s", json_str);
                        free(json_str);
                    }
                    
                    // Log specific fields from jobs/get/accepted response
                    printf("\n[OTA-JOBS] Extracted Fields:");
                    
                    // Check for timestamp (might be number)
                    cJSON *timestamp = cJSON_GetObjectItem(json, "timestamp");
                    if (timestamp) {
                        if (cJSON_IsNumber(timestamp)) {
                            printf("\n[OTA-JOBS]   timestamp: %.0f", cJSON_GetNumberValue(timestamp));
                        } else if (cJSON_IsString(timestamp)) {
                            const char *timestamp_str = cJSON_GetStringValue(timestamp);
                            if (timestamp_str) {
                                printf("\n[OTA-JOBS]   timestamp: %s", timestamp_str);
                            }
                        }
                    }
                    
                    // Check for clientToken
                    cJSON *clientToken = cJSON_GetObjectItem(json, "clientToken");
                    if (clientToken && cJSON_IsString(clientToken)) {
                        const char *clientToken_str = cJSON_GetStringValue(clientToken);
                        if (clientToken_str) {
                            printf("\n[OTA-JOBS]   clientToken: %s", clientToken_str);
                        }
                    }
                    
                    // Check for inProgressJobs array
                    cJSON *inProgressJobs = cJSON_GetObjectItem(json, "inProgressJobs");
                    if (inProgressJobs && cJSON_IsArray(inProgressJobs)) {
                        printf("\n[OTA-JOBS]   inProgressJobs array found with %d jobs", 
                               cJSON_GetArraySize(inProgressJobs));
                        
                        // Process each job in the array
                        cJSON *job;
                        int job_index = 0;
                        cJSON_ArrayForEach(job, inProgressJobs) {
                            if (cJSON_IsObject(job)) {
                                printf("\n[OTA-JOBS]   Job %d:", job_index + 1);
                                
                                cJSON *jobId = cJSON_GetObjectItem(job, "jobId");
                                if (jobId && cJSON_IsString(jobId)) {
                                    const char *jobId_str = cJSON_GetStringValue(jobId);
                                    if (jobId_str) {
                                        printf("\n[OTA-JOBS]     jobId: %s", jobId_str);
                                    }
                                }
                                
                                cJSON *queuedAt = cJSON_GetObjectItem(job, "queuedAt");
                                if (queuedAt) {
                                    if (cJSON_IsNumber(queuedAt)) {
                                        printf("\n[OTA-JOBS]     queuedAt: %.0f", cJSON_GetNumberValue(queuedAt));
                                    } else if (cJSON_IsString(queuedAt)) {
                                        const char *queuedAt_str = cJSON_GetStringValue(queuedAt);
                                        if (queuedAt_str) {
                                            printf("\n[OTA-JOBS]     queuedAt: %s", queuedAt_str);
                                        }
                                    }
                                }
                                
                                cJSON *lastUpdatedAt = cJSON_GetObjectItem(job, "lastUpdatedAt");
                                if (lastUpdatedAt) {
                                    if (cJSON_IsNumber(lastUpdatedAt)) {
                                        printf("\n[OTA-JOBS]     lastUpdatedAt: %.0f", cJSON_GetNumberValue(lastUpdatedAt));
                                    } else if (cJSON_IsString(lastUpdatedAt)) {
                                        const char *lastUpdatedAt_str = cJSON_GetStringValue(lastUpdatedAt);
                                        if (lastUpdatedAt_str) {
                                            printf("\n[OTA-JOBS]     lastUpdatedAt: %s", lastUpdatedAt_str);
                                        }
                                    }
                                }
                                
                                cJSON *startedAt = cJSON_GetObjectItem(job, "startedAt");
                                if (startedAt) {
                                    if (cJSON_IsNumber(startedAt)) {
                                        printf("\n[OTA-JOBS]     startedAt: %.0f", cJSON_GetNumberValue(startedAt));
                                    } else if (cJSON_IsString(startedAt)) {
                                        const char *startedAt_str = cJSON_GetStringValue(startedAt);
                                        if (startedAt_str) {
                                            printf("\n[OTA-JOBS]     startedAt: %s", startedAt_str);
                                        }
                                    }
                                }
                                
                                cJSON *executionNumber = cJSON_GetObjectItem(job, "executionNumber");
                                if (executionNumber && cJSON_IsNumber(executionNumber)) {
                                    printf("\n[OTA-JOBS]     executionNumber: %.0f", cJSON_GetNumberValue(executionNumber));
                                }
                                
                                cJSON *versionNumber = cJSON_GetObjectItem(job, "versionNumber");
                                if (versionNumber && cJSON_IsNumber(versionNumber)) {
                                    printf("\n[OTA-JOBS]     versionNumber: %.0f", cJSON_GetNumberValue(versionNumber));
                                }
                            }
                            job_index++;
                        }
                    ...
                    // Check for queuedJobs array
                    cJSON *queuedJobs = cJSON_GetObjectItem(json, "queuedJobs");
                    if (queuedJobs && cJSON_IsArray(queuedJobs)) {
                        printf("\n[OTA-JOBS]   queuedJobs array found with %d jobs", 
                               cJSON_GetArraySize(queuedJobs));
                    }
                ...
                // ========================================
                // OTHER OTA-RELATED TOPICS (for completeness)
                // ========================================
                if (strstr(topic, "ota/") != NULL || 
                    strstr(topic, "OTA/") != NULL ||
                    strstr(topic, "/jobs/") != NULL ||
                    strstr(topic, "/job/") != NULL) {
                    
                    printf("\n[OTA] ========================================");
                    printf("\n[OTA] OTA-RELATED MESSAGE (other topic)");
                    printf("\n[OTA] ========================================");
                    printf("\n[OTA] Topic: %.*s", event->topic_len, event->topic);
                    printf("\n[OTA] Payload Length: %d bytes", event->data_len);
                    
                    // Print first 500 chars of payload
                    if (event->data_len > 0) {
                        int print_len = (event->data_len < 500) ? event->data_len : 500;
                        printf("\n[OTA] Payload (first %d chars):", print_len);
                        printf("\n%.*s", print_len, event->data);
                    }
                    ...
                    printf("\n[OTA] JSON Structure:");
                    char *json_formatted = cJSON_Print(json);
                    if (json_formatted) {
                        printf("\n[OTA] %s", json_formatted);
                        free(json_formatted);
                    }
                    
                    printf("\n[OTA] ========================================");
                    
                    // Handle specific OTA topics
                    if (strstr(topic, "ota/job") != NULL) {
                        printf("\n[OTA] OTA Job Notification Received");
                        // Extract job ID, document, etc.
                        cJSON *jobId = cJSON_GetObjectItem(json, "jobId");
                        cJSON *operation = cJSON_GetObjectItem(json, "operation");
                        if (jobId) {
                            printf("\n[OTA] Job ID: %s", cJSON_GetStringValue(jobId));
                        }
                        if (operation) {
                            printf("\n[OTA] Operation: %s", cJSON_GetStringValue(operation));
                        }
                    } 
                    else if (strstr(topic, "ota/update") != NULL) {
                        printf("\n[OTA] OTA Update Command Received");
                        // Extract update parameters
                        cJSON *version = cJSON_GetObjectItem(json, "version");
                        cJSON *url = cJSON_GetObjectItem(json, "url");
                        if (version) {
                            printf("\n[OTA] Target Version: %s", cJSON_GetStringValue(version));
                        }
                        if (url) {
                            printf("\n[OTA] Update URL: %s", cJSON_GetStringValue(url));
                        }
                    }
...
static void subscribe_to_topics(void) {
    ...
    // ✅ Only subscribe to operational topics if fully registered
        ...
        // ========== OTA INITIALIZATION ==========
        printf("\n[OTA] Initializing OTA update system...");
        esp_err_t ota_ret = ota_job_init(thing_name, mqtt_client);
        if (ota_ret == ESP_OK) {
            printf("\n[OTA] OTA initialized successfully");
            
            // Subscribe to OTA job topics
            ota_ret = ota_job_subscribe();
            if (ota_ret == ESP_OK) {
                printf("\n[OTA] Subscribed to OTA job topics");
            } else {
                printf("\n[OTA] Failed to subscribe to OTA topics");
            }
        } else {
            printf("\n[OTA] Failed to initialize OTA system");
        }
        // ========================================
...
// OTA ALERT FUNCTION
// =======================
void send_ota_alert(const char *iostatus, const char *version)
{
    if (!mqtt_connected || !mqtt_client) {
        printf("\n[OTA] Cannot send alert - MQTT not connected");
        return;
    }
    
    printf("\n[OTA] Sending OTA alert - status: %s, version: %s", iostatus, version);
    ...
    if (root) {
        ...
        cJSON *payload_obj = cJSON_AddObjectToObject(root, "payload");
        if (payload_obj) {
            cJSON_AddStringToObject(payload_obj, "alerttype", "otaupdate");
            cJSON_AddStringToObject(payload_obj, "iostatus", iostatus);  // "start", "completed", "failed"
            cJSON_AddStringToObject(payload_obj, "version", version);
            cJSON_AddBoolToObject(payload_obj, "acknowledgement", false);
        }
        
        char *json_str = create_compact_json_string(root);
        if (json_str) {
            char topic[128];
            snprintf(topic, sizeof(topic), "Request/%s/Alert", mac_address);
            enqueue_mqtt_publish(topic, json_str);
            free(json_str);
            printf("\n[OTA] Alert queued for publishing");
        }
        cJSON_Delete(root);
    }
}
...
void display_system_status(void) {
    ...
    printf("\nOTA STATUS");
        ...
        printf("\n  Current Version: %s", app_desc->version);
        printf("\n  Compile Date: %s %s", app_desc->date, app_desc->time);

        if (ota_job_is_active()) {
            ota_job_info_t *info = ota_job_get_info();
            printf("\n  OTA Job: ACTIVE");
            printf("\n  New Version: %s", info->version);
            printf("\n  Progress: %d percent complete", info->progress_percent);
          } else {
            printf("\n  OTA Job: IDLE");
          }
}
